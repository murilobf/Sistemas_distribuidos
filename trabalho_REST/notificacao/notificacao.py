...
def callback_promocao_recebida(ch, method, properties, body):
    ...
    json_payload_promocao = json.dumps(payload_promocao, sort_keys=True).encode()
    ...
    try:
        ...
        if(payload_promocao.get('destaque')):
            msg = EmailMessage()
            msg.set_content(payload_promocao['nome'])
            msg['Subject'] = f'Promoção em destaque'
            msg['From'] = email
            msg['To'] = payload_promocao['email']
