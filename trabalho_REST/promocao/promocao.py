...
def callback_promocao_recebida(ch, method, properties, body):
    ...
    json_payload_gateway = json.dumps(payload_gateway, sort_keys=True).encode()

    #Verifica se a assinatura é válida
    try: 
        chave_publica_loja.verify(bytes.fromhex(assinatura_gateway), json_payload_gateway, padding.PKCS1v15(), hashes.SHA256())
...
with open('./chaves_publicas/chave_publica_loja.pem', 'rb') as f:
    chave_publica_loja = serialization.load_pem_public_key(f.read())
